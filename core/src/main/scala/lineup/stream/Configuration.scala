...
object Configuration {
  ...
  private object Directory {
    ...
    val DETECTION_BUDGET = "lineup.workflow.detect.timeout"
    val PLAN_PATH = "lineup.detection-plans"
    val TCP_INBOUND_BUFFER_SIZE = "lineup.source.buffer"
...
//      Req.withoutUsageSetting( Directory.PUBLISH_GRAPHITE_PORT ),
  ...
  final case class SimpleConfiguration private[stream](
    ...
    private def pullCommonPlanFacets( spec: Config ): (FiniteDuration, Set[Symbol]) = {
      ...
      (
        effectiveBudget( detectionBudget, 0.8D ), // FiniteDuration( spec.getDuration("timeout").toNanos, NANOSECONDS ),
  ...
  private object UsageSettings {
    ...
    def makeUsageConfig = new scopt.OptionParser[UsageSettings]( "lineup" ) {
