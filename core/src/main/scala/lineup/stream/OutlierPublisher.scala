    ...
    o match {
      ...
      case NoOutliers(_, source) => {
        ...
        @tailrec def fillInterval( timePoint: joda.DateTime, range: joda.Interval, acc: Seq[DataPoint] ): Seq[DataPoint] = {
          ...
          else fillInterval( timePoint + 1000L, range, acc :+ DataPoint(timePoint, 0D) )
        }

        source.interval map { i => fillInterval( i.start, i, Seq.empty[DataPoint] ) } getOrElse { Seq.empty[DataPoint] }
      }
