...
abstract class Algorithm[S <: Serializable: Advancing]
  ...
  case object module extends AggregateRootModule[State, Algorithm.ID] with ClassLogging {
    ...
    class RootType extends AggregateRootType {
      ...
      override val identifying: Identifying[S] = algorithm.identifying
...
//abstract class AlgorithmModule[S <: Serializable](
//    implicit
//    override val identifying: Identifying.Aux[AlgorithmState[S], AlgorithmModule.ID],
//    advancing: Advancing[S]
//) extends AggregateRootModule[AlgorithmState[S], AlgorithmModule.ID] with Instrumented with ClassLogging {
//  module: AlgorithmModule.ModuleConfiguration ⇒
//
//    import AlgorithmModule.{ Advancing, StrictSelf }
//  import AlgorithmProtocol.Advanced

//  override implicit lazy val identifying: Identifying.Aux[AlgorithmState[S], AlgorithmModule.ID] = new EntityIdentifying[AlgorithmState[S]] {
//    override lazy val idTag: Symbol = Symbol( algorithm.label )
//    override def nextTID: TryV[TID] = -\/( new IllegalStateException( "AlgorithmModule TIDs are created via AlgorithmRoutes" ) )
//    override def idFromString( idRep: String ): ID = Valid.unsafeGet( AlgorithmIdentifier.fromAggregateId( idRep ) )
//  }
...
//  val shapeCompanion: Advancing[Shape]
...
//todo pull out as stand-alone class? with common too
...
//        _advances += 1
//        val newState = cs.withTopicShape( event.topic, advancing.advance( s, event ) )
//
//        if ( AdvanceLimit < _advances ) {
//          log.debug( "advance limit exceed before snapshot time - taking snapshot with sweeps.." )
//          _advances = 0
...
//          accumulation = Accumulation(
//            topic = analysisContext.topic,
//            shape = state.shapes.get( analysisContext.topic ) getOrElse advancing.zero( Option( analysisContext.configuration ) )
//          ).right
//        )
//      }
//    }
